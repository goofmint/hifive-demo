...
(function($) {
	...
	var AllFetchSearcher = util.defineClass(NAMESPACE + '.AllFetchSearcher', function(ctx) {
		...
		var allFetchSearcherDefinition = {
			...
			_removeData: function(changeEvent, fromServer) {
				...
				var removedData = this._cache[index];

				// 削除扱いとする
				removedData.editStatus = 'removed';
				removedData.edited = null;
			},
	...
	// ---- LaxyFetchDataSearcher ---- //

	// TODO: LazyFetchDataSearcher
...
(function($) {
	...
	var tableGridViewController = {
		...
		'.gridCellFrame mousedown': function(context, $el) {
			...
			if (!$target.parents('.' + HEADER_COLUMNS_BOX_CLASS).length
					&& !$target.parents('.' + MAIN_BOX_CLASS).length) {
				// メインセルでない(ヘッダ)なら無視する
				return;
			}
...
(function($) {
	...
	var gridController = {
		...
		'{this._gridLogic} commitError': function() {
